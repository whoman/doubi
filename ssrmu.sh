...
check_root(){
[[ $EUID != 0 ]] && echo -e "${Error} The current account is not ROOT (or does not have ROOT permissions) and cannot continue the operation. Please use${Green_background_prefix} sudo su ${Font_color_suffix} to obtain temporary ROOT permissions (you will be prompted to enter the password of the current account after execution) " && exit 1.
}
check_sys(){
...
check_crontab(){
[[ ! -e "/usr/bin/crontab" ]] && echo -e "${Error} Missing dependency Crontab, please try to install CentOS manually: yum install crond -y, Debian/Ubuntu: apt-get install cron - y !" && exit 1
}
...
[[ -z ${forbidden_port} ]] && forbidden_port="no restrictions"
...
echo -e "obfuscation\t : ${Red_font_prefix}${obfs}${Font_color_suffix}"
...
echo -e " ${Green_font_prefix} prompt: ${Font_color_suffix}
  In the browser, open the QR code link and you can see the QR code image.
  [_compatible] after the protocol and obfuscation refers to compatibility with the original protocol/obfuscation. "
echo && echo "============================================== ====="
}
# Set configuration information
Set_config_user(){
echo "Please enter the username to be set (please do not repeat it, it is used to distinguish, Chinese and spaces are not supported, and an error will be reported!)"
...
while true
do
echo -e "Please enter the user port to be set (do not repeat, for differentiation)"
...
echo -e "Please select the user encryption method to be set
  ...
  ${Tip} If you use the auth_chain_* series of protocols, it is recommended to select none for the encryption method (this series of protocols comes with RC4 encryption), and confusion is free.
  ...
  ${Tip} salsa20/chacha20-* series encryption methods require additional installation of the dependency libsodium, otherwise ShadowsocksR will not be started!" && echo
...
Set_config_protocol(){
echo -e "Please select the user protocol plug-in to set
  ...
  ${Tip} If you use the auth_chain_* series of protocols, it is recommended to select none for the encryption method (this series of protocols comes with RC4 encryption), and you can be confused as you like" && echo
...
if [[ ${ssr_protocol} == "auth_sha1_v4" ]]; then
			read -e -p "Do you want to set the protocol plug-in to be compatible with the original version (_compatible)? [Y/n]" ssr_protocol_yn
...
Set_config_obfs(){
echo -e "Please select the user to set up the obfuscation plug-in
  ...
  ${Tip} If you use ShadowsocksR proxy game, it is recommended to select obfuscation compatible with the original version or plain obfuscation, and then select plain on the client side, otherwise the delay will increase!
  In addition, if you select tls1.2_ticket_auth, the client can select tls1.2_ticket_fastauth, so that it can be disguised without increasing the delay!
  If you are building in popular areas such as Japan and the United States, then choosing plain obfuscation may have a lower chance of being blocked!" && echo
...
echo && echo ${Separator_1} && echo -e " obfuscation: ${Green_font_prefix}${ssr_obfs}${Font_color_suffix}" && echo ${Separator_1} && echo
if [[ ${ssr_obfs} != "plain" ]]; then
read -e -p "Do you want to set the obfuscation plug-in to be compatible with the original version (_compatible)? [Y/n]" ssr_obfs_yn
...
while true
do
echo -e "Please enter the user to be set and the number of devices to be restricted (${Green_font_prefix} auth_* series protocols are only valid if they are incompatible with the original version ${Font_color_suffix})"
echo -e "${Tip} device number limit: the number of clients that can be connected to each port at the same time (multi-port mode, each port is calculated independently), at least 2 are recommended."
...
while true
do
echo -e "Please enter the user to set the single-thread speed limit limit (unit: KB/S)"
echo -e "${Tip} Single-thread speed limit: The upper limit of single-thread speed limit for each port. Multi-threading is invalid."
...
do
echo
echo -e "Please enter the user's total speed and speed limit limit to be set (unit: KB/S)"
echo -e "${Tip} Port total speed limit: the total speed of each port is the upper limit of the speed limit, and the overall speed limit of a single port."
...
do
echo
echo -e "Please enter the total traffic limit that can be used by the user to be set (unit: GB, 1-838868 GB)"
...
Set_config_forbid(){
echo "Please enter the user to be set and the port that is prohibited to access"
echo -e "${Tip} Banned ports: For example, if access to port 25 is not allowed, users will not be able to access email port 25 through the SSR proxy. If 80,443 is banned, users will not be able to access http/https websites normally.
Format of blocking a single port: 25
Block multiple ports format: 23,465
Banned port segment format: 233-266
Block multiple formats of ports: 25,465,233-666 (without colon:)"
read -e -p "(default is empty, does not prohibit access to any port):" ssr_forbid
...
[[ "${ssr_port_num}" == "null" ]] && echo -e "${Error} failed to obtain the row number of the current port [${ssr_port}]!" && exit 1
...
if [[ "${enable}" == "1" ]]; then
echo -e "The account status of port [${ssr_port}] is: ${Green_font_prefix} enables ${Font_color_suffix}, whether to switch to ${Red_font_prefix} and disables ${Font_color_suffix}? [Y/n]"
...
echo -e "The account status of port [${ssr_port}] is: ${Green_font_prefix} disables ${Font_color_suffix}, do you want to switch to ${Red_font_prefix} and enable ${Font_color_suffix}? [Y/n]"
read -e -p "(Default: Y):" ssr_enable_yn
[[ -z "${ssr_enable_yn}" ]] && ssr_enable_yn = "y"
if [[ "${ssr_enable_yn}" == [Yy] ]]; then
ssr_enable="1"
...
read -e -p "(default automatically detects external network IP):" ssr_server_pub_addr
if [[ -z "${ssr_server_pub_addr}" ]]; then
Get_IP
if [[ ${ip} == "VPS_IP" ]]; then
while true
do
read -e -p "${Error} failed to automatically detect the external network IP, please enter the server IP or domain name manually" ssr_server_pub_addr
if [[ -z "$ssr_server_pub_addr" ]]; then
...
echo -e "${Info} user password modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
...
echo -e "${Info} user encryption method modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
...
echo -e "${Info} user agreement modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
Modify_config_obfs(){
...
echo -e "${Info} user obfuscation modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
Modify_config_protocol_param(){
match_edit=$(python mujson_mgr.py -e -p "${ssr_port}" -G "${ssr_protocol_param}"|grep -w "edit user ")
if [[ -z "${match_edit}" ]]; then
echo -e "${Error} User protocol parameters (device limit) modification failed ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} " && exit 1
else
echo -e "${Info} user parameters (limit on the number of devices) have been modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
...
echo -e "${Info} user single-thread speed limit modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
...
echo -e "${Info} user port total speed limit modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
...
echo -e "${Info} total user traffic modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
Modify_config_forbid(){
match_edit=$(python mujson_mgr.py -e -p "${ssr_port}" -f "${ssr_forbid}"|grep -w "edit user ")
	if [[ -z "${match_edit}" ]]; then
echo -e "${Error} User forbidden access port modification failed ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} " && exit 1
else
echo -e "${Info} user prohibited access port has been modified successfully ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Note: It may take about ten seconds for the latest configuration to be applied)"
fi
}
...
echo -e "${Info} Python is not installed, start installing..."
if [[ ${release} == "centos" ]]; then
...
echo -e "${Info} JQ parser has been installed, continue..."
fi
}
...
[[ ! -e "/usr/bin/unzip" ]] && echo -e "${Error} relies on unzip (decompression package). The installation failed. It is probably a problem with the software package source. Please check!" && exit 1
Check_python
...
Install_SSR(){
check_root
[[ -e ${ssr_folder} ]] && echo -e "${Error} ShadowsocksR folder already exists, please check (if the installation fails or an old version exists, please uninstall it first)!" && exit 1
...
Download_SSR
echo -e "${Info} starts downloading/installing ShadowsocksR service script (init)..."
Service_SSR
echo -e "${Info} starts downloading/installing JSNO parser JQ..."
JQ_install
echo -e "${Info} starts adding initial users..."
Add_port_user "install"
echo -e "${Info} Start setting up iptables firewall..."
Set_iptables
echo -e "${Info} Start adding iptables firewall rules..."
Add_iptables
echo -e "${Info} starts saving iptables firewall rules..."
Save_iptables
...
Update_SSR(){
SSR_installation_status
echo -e "Because Powa has suspended updating the ShadowsocksR server, this feature is temporarily disabled."
#cd ${ssr_folder}
#git pull
#Restart_SSR
}
...
Check_Libsodium_ver(){
echo -e "${Info} starts to get the latest version of libsodium..."
...
user_info=$(python mujson_mgr.py -l)
user_total=$(echo "${user_info}"|wc -l)
[[ -z ${user_info} ]] && echo -e "${Error} user not found, please check!" && exit 1
...
user_list_all=${user_list_all}"Username: ${Green_font_prefix}"${user_info_233}"${Font_color_suffix}\t Port: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t Total number of link IPs: ${ Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t Current link IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
user_IP=""
done
echo -e "Total number of users: ${Green_background_prefix} "${user_total}" ${Font_color_suffix} Total number of link IPs: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix} "
...
View_user_connection_info(){
	SSR_installation_status
echo && echo -e "Please select the format to be displayed:
...
echo -e "${Tip} Check the IP address (ipip.net). If there are more IPs, it may take longer..."
...
———— Modify user configuration ————
  ...
  ${Green_font_prefix}5.${Font_color_suffix} Modify protocol plug-in
  ${Green_font_prefix}6.${Font_color_suffix} Modify obfuscation plug-in
  ${Green_font_prefix}7.${Font_color_suffix} Modify device number limit
...
----- other -----
  ...
  ${Tip} The user name and port cannot be modified. If you need to modify it, please use the manual modification function of the script!" && echo
...
echo -e ${user_list_all}
echo -e "=== The total traffic used by all current users: ${Green_background_prefix} ${transfer_enable_Used_233_2} ${Font_color_suffix}\n"
}
Add_port_user(){
lalal=$1
...
echo && echo -e "What do you want to do?
  ...
  ${Green_font_prefix}2.${Font_color_suffix} Clear the traffic used by all users (irreversible)
  ${Green_font_prefix}3.${Font_color_suffix} Start clearing all user traffic regularly
  ${Green_font_prefix}4.${Font_color_suffix} Stop Clear all user traffic regularly
  ${Green_font_prefix}5.${Font_color_suffix} Modify Clear all user traffic regularly" && echo
read -e -p "(default: cancel):" ssr_modify
[[ -z "${ssr_modify}" ]] && echo "Cancelled..." && exit 1
if [[ ${ssr_modify} == "1" ]]; then
Clear_transfer_one
...
while true
do
echo -e "Please enter the user port that you want to clear the used traffic"
...
echo -e "${Error} Failed to clear user's used traffic ${Green_font_prefix}[Port: ${Clear_transfer_user_port}]${Font_color_suffix} "
else
echo -e "${Info} user traffic has been cleared successfully ${Green_font_prefix}[Port: ${Clear_transfer_user_port}]${Font_color_suffix} "
fi
break
else
echo -e "${Error} Please enter the correct port!"
...
if [[ -z "${match_clear}" ]]; then
echo -e "${Error} Failed to clear the traffic used by the user ${Green_font_prefix}[Port: ${user_port}]${Font_color_suffix} "
else
echo -e "${Info} user traffic has been cleared successfully ${Green_font_prefix}[Port: ${user_port}]${Font_color_suffix} "
fi
done
echo -e "${Info} All user traffic has been cleared!"
}
Clear_transfer_all_cron_start(){
...
if [[ -z ${cron_config} ]]; then
		echo -e "${Error} scheduled all user traffic clearing startup failed!" && exit 1
else
echo -e "${Info} scheduled all user traffic clearing started successfully!"
fi
}
Clear_transfer_all_cron_stop(){
crontab -l > "$file/crontab.bak"
...
cron_config=$(crontab -l | grep "ssrmu.sh")
if [[ ! -z ${cron_config} ]]; then
echo -e "${Error} Failed to clear and stop scheduled all user traffic!" && exit 1
else
...
echo -e "Please enter the traffic clearing interval
  === Format description ===
  * * * * * Corresponds to minutes, hours, days, months, and weeks, respectively.
  ${Green_font_prefix} 0 2 1 * * ${Font_color_suffix} means clearing the used traffic at 2:00 on the 1st of every month
  ${Green_font_prefix} 0 2 15 * * ${Font_color_suffix} means clearing the used traffic at 2:00 on the 15th of every month
  ${Green_font_prefix} 0 2 */7 * * ${Font_color_suffix} means clearing the used traffic every 7 days at 2:00
  ${Green_font_prefix} 0 2 * * 0 ${Font_color_suffix} represents clearing the used traffic every Sunday (7)
  ${Green_font_prefix} 0 2 * * 3 ${Font_color_suffix} represents clearing the used traffic every Wednesday (3)" && echo
...
echo && echo -e "What do you want to do?
  ${Green_font_prefix}1.${Font_color_suffix} Installation Ruisu
  ${Green_font_prefix}2.${Font_color_suffix} Uninstall Ruisu
————————
  ${Green_font_prefix}3.${Font_color_suffix} Start Ruisu
  ${Green_font_prefix}4.${Font_color_suffix} Stop Rapid
...
echo && echo "LotServer uninstallation completed!" && echo
fi
}
#BBR
Configure_BBR(){
...
1. To install and enable BBR, you need to replace the kernel, and there are risks such as replacement failure (cannot boot after restarting)
2. This script only supports kernel replacement in Debian/Ubuntu systems. OpenVZ and Docker do not support kernel replacement.
3. During Debian kernel replacement, you will be prompted [Whether to terminate kernel uninstallation], please select ${Green_font_prefix} NO ${Font_color_suffix}" && echo
...
echo && echo -e "What do you want to do?
   ...
   ${Tip} Ruisu and LotServer cannot coexist!
————————————
...
————————————
   ...
   —— Description: SSR only outputs error logs by default. This item can be switched to output detailed access logs.
   ${Green_font_prefix}7.${Font_color_suffix} Monitor the running status of ShadowsocksR server
   —— Description: This function is suitable for the frequent process termination of the SSR server. After starting this function, it will be detected once every minute. When the process does not exist, the SSR server will be automatically started. " && echo
...
echo && echo -e "Current log mode: ${Green_font_prefix} simple mode (only error logs are output) ${Font_color_suffix}" && echo
...
echo -e "Are you sure you want to enable the ${Green_font_prefix}ShadowsocksR server running status monitoring ${Font_color_suffix} function? (When the process is closed, the SSR server will be automatically started) [Y/n]"
...
echo && echo -e "Current monitoring mode: ${Green_font_prefix} is turned on ${Font_color_suffix}" && echo
echo -e "Are you sure you want to turn off the ${Green_font_prefix}ShadowsocksR server running status monitoring ${Font_color_suffix} function? (When the process is closed, the SSR server will be automatically started) [y/N]"
...
check_pid
if [[ -z ${PID} ]]; then
echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] It is detected that the ShadowsocksR server is not running and starts to start... " | tee -a ${ssr_log_file}
/etc/init.d/ssrmu start
sleep 1s
check_pid
if [[ -z ${PID} ]]; then
...
echo -e "The script has been updated to the latest version [${sh_new_ver}]! (Note: Because the update method directly overwrites the currently running script, some errors may be prompted below, just ignore them)" && exit 0
}
# Show menu status
menu_status(){
...
check_pid
if [[ ! -z "${PID}" ]]; then
echo -e "Current status: ${Green_font_prefix} has installed ${Font_color_suffix} and ${Green_font_prefix} has started ${Font_color_suffix}"
else
...
————————————
   ...
   ${Green_font_prefix}9.${Font_color_suffix} configuration traffic clearing
————————————
...
————————————
  ...
  "
menu_status
echo && read -e -p "Please enter a number [1-15]:" num
case "$num" in
	1)
Install_SSR
;;
